...
int sysPressed(int key){
  return GetAsyncKeyState(key) & 0x8000;
}

int sysTapped(int key){
...
void setMousePos(struct mouseHandler *mouseH, int pos){
  mouseH->pos = pos;
}
...
void clearMouseHandler(struct mouseHandler *mouseH){
  mouseH->button = 0;
}
...
void setPaintPen(struct paintHandler *paintH, UINT style, UINT width, UINT color){
  if((paintH->penStyle != style) || (paintH->penWidth != width) || (paintH->penColor != color)){
    if(paintH->pen) DeleteObject(paintH->pen);
...
void setPaintBrush(struct paintHandler *paintH, UINT color){
  if(paintH->brushColor != color){
    if(paintH->brush) DeleteObject(paintH->brush);
    paintH->brush = CreateSolidBrush(color);
    paintH->brushColor = color;
  }
}

Register CALLBACK procedure(Window, UINT code, size_t flag, Register);
...
Window newWindow(WindowFrame *frame, const char *name, int x, int y, int w, int h){
  Window window = CreateWindowEx(0, frame->class.lpszClassName, name, WS_TILEDWINDOW, x, y, w, h, 0, 0, frame->class.hInstance, 0);
...
Register CALLBACK procedure(Window window, UINT code, size_t flag, Register data){
  ...
  case WM_SETCURSOR:{
    ...
    if(cursorChanged) updateCursor(&frame->curH);
    break;
  }
  case WM_PAINT:{
    WindowFrame *frame = (WindowFrame*)GetWindowLongPtr(window, GWLP_USERDATA);
